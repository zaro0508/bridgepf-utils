...
public class BridgeSpringConfig {
    ...
    public AmazonDynamoDBClient dynamoDbClient() {
        ...
        if (awsCredentials().getAWSAccessKeyId().equals("AWS")) {
            LOG.info("DDB Using Instance Profile");
            return new AmazonDynamoDBClient(new InstanceProfileCredentialsProvider(), awsClientConfig);
        } else {
            LOG.info("DDB Using Static Creds");
